      ...
      <div className="max-w-2xl mx-auto px-6 text-center">
        <FadeIn>
          ...
          <h2 className="font-mincho text-[28px] md:text-[34px] text-ink-900 leading-[1.6] mb-6">
            まずは、<span className="text-gold-600">渋谷院</span>へ。
          </h2>
        ...
        <FadeIn delay={0.15}>
          <a
            ...
            className="inline-flex items-center justify-center rounded-pill bg-gradient-to-br from-gold-500 to-gold-600 text-white font-bold text-[15px] tracking-[0.03em] px-10 py-4 shadow-soft hover:from-gold-600 hover:to-gold-500 hover:-translate-y-0.5 transition-all duration-300 mb-14"
          >
            無料カウンセリングを予約する
          </a>
        </FadeIn>
          ...
          <div className="bg-cream-50 rounded-card border border-gold-500/40 shadow-card text-left divide-y divide-gold-500/15 overflow-hidden">
            {rows.map(([label, value]) => (
              <div key={label} className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-6 px-7 py-5">
                <dt className="sm:w-24 flex-shrink-0 text-[12px] tracking-[0.1em] text-gold-600">{label}</dt>
                <dd className="text-[14px] text-ink-900">{value}</dd>
              </div>
            ))}
          </div>
        </FadeIn>
